        ...
        try {
            ...
            if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if (jwtService.isTokenValid(token, userDetails)) {
                    ...
                    LOGGER.debug("JWT authenticated user {} on {}", username, request.getRequestURI());
                }
            }
        } catch (RuntimeException exception) {
            // Malformed, expired, unknown-user or tampered tokens must not leak details.
            LOGGER.debug("JWT rejected on {}: {}", request.getRequestURI(), exception.getClass().getSimpleName());
